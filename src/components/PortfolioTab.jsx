...
function MoneyCell({ v, mon, colored }) {
  ...
  return <span className={`td-mono ${cls}`} style={{ whiteSpace: 'nowrap' }}>{sign}{pfx}{val}</span>;
}
...
function VActualCell({ v, costo, mon }) {
  ...
  return <span className={`td-mono ${cls}`} style={{ whiteSpace: 'nowrap', fontWeight: 600 }}>{pfx}{val}</span>;
}

function PctCell({ v, d = 2 }) {
  if (v === null || isNaN(v)) return <span className="td-mono zero">—</span>;
  const cls = v > 0 ? 'pos' : v < 0 ? 'neg' : 'zero';
  return <span className={`td-mono ${cls}`}>{v > 0 ? '+' : ''}{v.toFixed(d)}%</span>;
}
...
export default function PortfolioTab({ raw, apiFecha, refs, onOpenSuscripcion, onOpenRescate, onOpenHistorial, onFondoClick }) {
  ...
  const clearFilters = () => { setQ(''); setFilterMoneda(''); };
    ...
    return (
      <div className="table-wrap">
        <div className="tbl-scroll">
          <table id="main-table">
            <tbody>
              <tr><td colSpan={14}><div className="empty-favs">
  ...
  const enriched = [...portfolioFiltered].sort((a, b) => a.fondo_nombre.localeCompare(b.fondo_nombre, 'es')).map(item => {
  ...
  const enrichedHistorico = fondosHistoricos.map(fn =>
    enriched.find(i => i.fondo_nombre === fn)
  ).filter(Boolean);
  ...
  return (
    <>
      {/* KPIs */}
      <div className="kpis">
        <div className="kpi"><div className="kpi-bar c"></div>
        ...
        {totalARS > 0 && <div className="kpi"><div className="kpi-bar c"></div>
          <div className="kpi-label">Invertido ARS</div>
          <div className="kpi-val c" style={{ fontSize: 18 }}>{fmtMoney(totalARS, 'ARS')}</div>
          <div className="kpi-sub">costo de compra</div>
        </div>}
        {rendARS !== 0 && vARS > 0 && <div className="kpi"><div className={`kpi-bar ${clsARS}`}></div>
          <div className="kpi-label">Rendimiento ARS</div>
          <div className={`kpi-val ${clsARS}`} style={{ fontSize: 18 }}>{rendARS > 0 ? '+' : ''}{fmtMoney(rendARS, 'ARS')}</div>
          <div className="kpi-sub">{rendARSpct !== null ? (rendARSpct > 0 ? '+' : '') + rendARSpct.toFixed(2) + '% sobre costo' : ''}</div>
        </div>}
        {totalUSD > 0 && <div className="kpi"><div className="kpi-bar c"></div>
          <div className="kpi-label">Invertido USD</div>
          <div className="kpi-val c" style={{ fontSize: 18 }}>{fmtMoney(totalUSD, 'USD')}</div>
          <div className="kpi-sub">costo de compra</div>
        </div>}
        {rendUSD !== 0 && vUSD > 0 && <div className="kpi"><div className={`kpi-bar ${clsUSD}`}></div>
          <div className="kpi-label">Rendimiento USD</div>
          <div className={`kpi-val ${clsUSD}`} style={{ fontSize: 18 }}>{rendUSD > 0 ? '+' : ''}{fmtMoney(rendUSD, 'USD')}</div>
      ...
      <div style={{ display: 'flex', gap: 6, margin: '0 0 14px' }}>
        <button
          ...
          style={{ padding: '5px 14px', borderRadius: 20, border: '1px solid var(--border)', fontSize: 12, fontWeight: 600, cursor: 'pointer', background: subTab === 'actual' ? 'var(--accent)' : 'transparent', color: subTab === 'actual' ? '#fff' : 'var(--text2)', transition: 'all 0.15s' }}>
          Tenencia Actual {enrichedActual.length > 0 && <span style={{ opacity: 0.8 }}>({enrichedActual.length})</span>}
        </button>
        <button
          onClick={() => setSubTab('historico')}
          style={{ padding: '5px 14px', borderRadius: 20, border: '1px solid var(--border)', fontSize: 12, fontWeight: 600, cursor: 'pointer', background: subTab === 'historico' ? 'var(--accent)' : 'transparent', color: subTab === 'historico' ? '#fff' : 'var(--text2)', transition: 'all 0.15s' }}>
      ...
      <div className="filter-bar">
        <div className="search-wrap">
          ...
          <input type="search" placeholder="Buscar fondo…" autoComplete="off"
        ...
        <button className="btn-informe-print" style={{ marginLeft: 'auto' }} onClick={() => setShowInforme(true)}>📄 Exportar PDF</button>
      </div>
          ...
          <table id="main-table">
            <thead>
              <tr>
                ...
                <th style={{ width: 100 }}>Cuota-<br />partes</th>
                <th style={{ width: 110 }}>P. Suscrip.<br />Pond.</th>
                <th style={{ width: 110 }}>Importe<br />Suscrip.</th>
                <th style={{ width: 110 }}>
                  Valor Actual
                  {apiFecha && <div style={{ fontSize: 9, fontWeight: 400, color: 'var(--muted2)', marginTop: 2, fontFamily: "'JetBrains Mono', monospace" }}>{(() => { const f = apiFecha; if (f && f.includes('-')) { const [y,m,d] = f.split('-'); return `${d}/${m}/${y.slice(2)}`; } return f; })()}</div>}
                </th>
                <th style={{ width: 80 }}>Rend.<br />%</th>
                <th style={{ width: 110 }}>Rendimiento<br />Importe</th>
                <th style={{ width: 80 }}>
                  Var. Diaria %
                  {refs?.fechaAnterior && <div style={{ fontSize: 9, fontWeight: 400, color: 'var(--muted2)', marginTop: 2, fontFamily: "'JetBrains Mono', monospace" }}>{refs.fechaAnterior}</div>}
                </th>
              ...
              {enrichedView.length === 0 ? (
                <tr><td colSpan={14}><div className="empty-favs">
                  {subTab === 'actual' ? (
                    <>
                      <div className="empty-icon">📊</div>
                      <p>No tenés suscripciones activas.<br />
                        <button className="btn-retry" style={{ marginTop: 14 }} onClick={() => onOpenSuscripcion()}>+ Agregar suscripción</button>
                      </p>
                    </>
                  ) : (
                    <>
                    ...
                    <td style={{ verticalAlign: 'middle' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, width: 52 }}>
                        ...
                        <button
                          ...
                          onClick={() => { const match = raw.find(r => r[1] === item.fondo_nombre); if (match) onFondoClick && onFondoClick(match); }}
                          title="Ver detalle del fondo"
                        >ⓘ</button>
                        <button className="btn-port-action btn-hist"
                          onClick={() => onOpenHistorial(item.fondo_nombre, item.moneda)}
                          title="Ver historial">📋</button>
                        {/* Fila 2: + y − (actual) o + y 🗑 (historico) */}
                        <button className="btn-port-action btn-susc"
                          onClick={() => onOpenSuscripcion(item.fondo_nombre, item.moneda)}
                          title="Nueva suscripción">+</button>
                        {subTab === 'actual' ? (
                          <button className="btn-port-action btn-resc"
                            onClick={() => onOpenRescate(item.fondo_nombre, item.moneda)}
                            title="Registrar rescate">−</button>
                        ) : (
                          <button className="btn-port-action btn-del"
                            onClick={() => confirmDelete(item)}
                            title="Eliminar posición">🗑</button>
                        )}
                      </div>
                    </td>
                    <td className="td-fondo" style={{ minWidth: 160, maxWidth: 220, whiteSpace: "normal", wordBreak: "break-word", lineHeight: 1.3 }}>
                      {item.fondo_nombre}
                      {!r && <span style={{ fontSize: 10, color: 'var(--yellow)' }} title="Sin match"> ⚠</span>}
                    </td>
                    <td><MonBadge m={item.moneda} /></td>
                    <td><span className="td-mono" style={{ fontSize: 11 }}>{cantFmt}</span></td>
                    <td><span className="td-mono" style={{ fontSize: 11 }}>{precFmt}</span></td>
                    <td><MoneyCell v={item.costoTotal} mon={item.moneda} /></td>
                    <td><VActualCell v={item.vActual} costo={item.costoTotal} mon={item.moneda} /></td>
                    <td><PctCell v={item.rendPct} /></td>
                    <td><MoneyCell v={item.rendImporte} mon={item.moneda} colored /></td>
                    <td><PctCell v={item.varDiariaPct} /></td>
                    <td><PctCell v={item.tna} /></td>
                    <td><PctCell v={item.tem} /></td>
                    <td><PctCell v={item.tea} /></td>
                    <td>{r ? <a className="btn-cnv" href={cnvLink(r[13] || '', r[0] || '')} target="_blank" rel="noopener" title="Ver en CAFCI">↗</a> : '—'}</td>
                  </tr>
                );
              })}
            </tbody>
